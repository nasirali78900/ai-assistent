...
export default function PermissionModal({ onClose }: Props) {
  const requestPermission = async () => {
    try {
      ...
      // If successful, we can just reload or close the modal
      window.location.reload();
    } catch (err) {
      console.error("Permission still denied:", err);
      alert("Microphone access is still blocked. Please enable it in your browser settings.");
    }
  };
      ...
      >
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-left w-full mb-8">
          ...
          <ol className="text-xs text-white/60 list-decimal pl-4 space-y-2">
            ...
            {isIframe && (
              <li>If you don't see the prompt, try <strong>opening the app in a new tab</strong> using the icon at the top right of the preview.</li>
            )}
            <li>If you've allowed it, click <strong>Try Again</strong> below.</li>
          </ol>
        </div>
