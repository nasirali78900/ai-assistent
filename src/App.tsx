...
export default function App() {
  ...
  const toggleListening = async () => {
    if (isSessionActive) {
      ...
      try {
        // 1. Request Microphone Access FIRST (must be directly in user gesture handler)
        console.log("Requesting microphone access from App.tsx...");
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
          throw new Error("Microphone access is not supported in this browser or context (e.g. non-HTTPS).");
        }
        ...
        console.log("Microphone access granted in App.tsx");
        ...
        // 2. Start session with the already obtained stream
        ...
        if (errorName === 'NotAllowedError' || errorName === 'PermissionDeniedError' || errorMsg.toLowerCase().includes('permission denied')) {
          ...
          alert(`Failed to start voice session: ${errorMsg}`);
        }
        setIsSessionActive(false);
        setAppState("idle");
      }
